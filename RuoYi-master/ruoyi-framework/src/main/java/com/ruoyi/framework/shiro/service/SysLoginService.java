...
public class SysLoginService
{
    ...
    /**
     * 手机号码登录，如果是新用户直接生成用户信息
     */
    public SysUser loginByMobile(String mobile)
    {
        ...
        if (user == null)
        {
            ...
            // 设置默认密码为手机号后6位
            String defaultPassword = mobile.substring(mobile.length() - 6);
            user.setPassword(passwordService.encryptPassword(user.getUserName(), defaultPassword, user.getSalt()));
    ...
    public SysUser loginByOpenId(String openId)
    {
        ...
        {
            AsyncManager.me().execute(AsyncFactory.recordLogininfor(openId, Constants.LOGIN_FAIL, MessageUtils.message("not.null")));
            throw new UserNotExistsException();
        }
        ...
        // 为避免用户名长度超过限制，使用openid的哈希值作为用户名
        String userIdentifier = "wx_" + openId.hashCode();
        // 查询用户信息 - 使用用户名字段存储openid哈希标识
        SysUser user = userService.selectUserByLoginName(userIdentifier); // 使用哈希标识微信用户
        ...
        if (user == null)
        {
            user = new SysUser();
            user.setSalt(ShiroUtils.randomSalt());
            user.setUserName(userIdentifier); // 使用哈希标识微信用户
            user.setNickName("微信用户_" + openId.substring(0, 8)); // 设置默认昵称
            // 设置默认密码
            String defaultPassword = "wx123456";
            user.setPassword(passwordService.encryptPassword(user.getUserName(), defaultPassword, user.getSalt()));
            // 注意：如果数据库中没有专门存储微信openid的字段，这里可以使用remark或其他字段临时存储
            // 或者在实际项目中需要添加专门的字段来存储微信相关标识
            userService.insertUser(user);
        }
     ...
     * @return 处理后的用户信息
     */
    private SysUser processUserLogin(SysUser user)
    {
        if (UserStatus.DELETED.getCode().equals(user.getDelFlag()))
        {
